...
const (
	// MaxMessageStateIDs is the MTProto limit for msgs_state_req and
	// msg_resend_req vectors.
	MaxMessageStateIDs = 8192

	// Canonical one-byte msgs_state_info values and additive flags.
	MessageStateUnknownTooOld     byte = 1
	MessageStateNotReceived       byte = 2
	MessageStateUnknownTooHigh    byte = 3
	MessageStateReceived          byte = 4
	MessageStateAcknowledged      byte = 8
	MessageStateNoAcknowledgement byte = 16
	MessageStateProcessing        byte = 32
	MessageStateResponseGenerated byte = 64
	MessageStateKnown             byte = 128
)
